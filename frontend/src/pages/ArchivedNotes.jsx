...
const ArchivedNotes = () => {
  ...
  const fetchNotes = useCallback(async () => {
    setLoading(true);
    try {
      const res = await noteService.getAll({ isArchived: 'true' });
      setNotes(res.data || res);
    } catch {
      addToast('Failed to load archived notes', 'error');
    } finally {
      setLoading(false);
    }
  }, [addToast]);
  ...
  const handleArchive = async (id) => {
    try {
      ...
      addToast('Note restored from archive', 'success');
      fetchNotes();
    } catch {
    ...
    <div className="page-container">
      ...
      <div className="page-header">
        <div className="page-header-left">
          <h1 className="page-title">
            <HiOutlineArchiveBox className="page-title-icon" />
            Archived Notes
            {!loading && (
      ...
      {loading ? (
        ...
        <div className="empty-state">
          <HiOutlineArchiveBox className="empty-state-icon" />
          <h2 className="empty-state-title">No archived notes</h2>
